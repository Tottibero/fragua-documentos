...
// caracteres, sin "/", "\" ni caracteres de control. Idénticas a las de
// UploadFileDialog — el archivo local que reemplaza el contenido pasa por
// la misma validación de nombre que cualquier subida.
...
const props = withDefaults(
  defineProps<{
    open: boolean
    /** Archivo cuyo contenido se va a reemplazar — `null` solo antes de la
     * primera apertura. Mientras `open` es `true` el llamador lo mantiene
     * estable (no lo limpia hasta que el diálogo se cierra de verdad). */
    item: DriveItem | null
...
// Mismo mecanismo que ConfirmDialog/UploadFileDialog: distingue un cierre
...
// se bloquea mientras se está reemplazando para no dejar la operación
// huérfana.
function handleNativeCancel(event: Event) {
  if (props.isReplacing) {
    event.preventDefault()
  }
}

function handleBackdropClick(event: MouseEvent) {
  if (!props.isReplacing && event.target === dialogEl.value) {
    emit('cancel')
  }
}
...
async function handleSubmit() {
  if (props.isReplacing || !hasValidUploadLimit.value) return
...
<template>
  <dialog
    ref="dialogEl"
    class="replace-file-dialog"
    aria-labelledby="replace-file-title"
    @close="handleClose"
    @cancel="handleNativeCancel"
    @click="handleBackdropClick"
  >
    <div class="replace-file-dialog__box" @click.stop>
      <h2 id="replace-file-title" class="replace-file-dialog__title">
        Subir nueva versión de «{{ item?.name ?? '' }}»
      </h2>

      <p class="replace-file-dialog__notice">Conservará el mismo nombre y ubicación en Drive.</p>

      <LoadingSpinner v-if="isLoadingConfig" label="Comprobando el límite de subida…" />

      <div v-else-if="showConfigUnavailable" class="replace-file-dialog__config-error">
        <AlertMessage variant="error">{{ configErrorMessage }}</AlertMessage>
        <button
          ref="retryConfigButton"
          type="button"
          class="replace-file-dialog__retry-config"
      ...
      <form
        ...
        class="replace-file-dialog__form"
        novalidate
        @submit.prevent="handleSubmit"
      >
        <p class="replace-file-dialog__limit">
          Límite por archivo: {{ formatBytes(maxUploadBytes ?? 0) }}
        </p>

        <div
          class="replace-file-dialog__dropzone"
          :class="{ 'replace-file-dialog__dropzone--active': isDragOver }"
          @dragover="handleDragOver"
          @dragleave="handleDragLeave"
          @drop="handleDrop"
        >
          <p class="replace-file-dialog__dropzone-hint">Arrastra el archivo nuevo aquí o</p>
          <button
            ref="chooseFileButton"
            type="button"
            class="replace-file-dialog__choose-button"
        ...
        <p v-if="selectedFile" class="replace-file-dialog__selected">
          <span class="replace-file-dialog__selected-name">{{ selectedFile.name }}</span>
          <span class="replace-file-dialog__selected-size">{{ formatBytes(selectedFile.size) }}</span>
        </p>

        <span v-if="fieldError" role="alert" class="field-error">{{ fieldError }}</span>

        <AlertMessage v-if="replaceError" variant="error">{{ replaceError }}</AlertMessage>
      </form>
      ...
      <div class="replace-file-dialog__actions">
        <button
          ref="cancelButton"
          type="button"
          class="replace-file-dialog__cancel"
          :disabled="isReplacing"
          @click="emit('cancel')"
        >
          Cancelar
        ...
        <button
          ...
          type="submit"
          form="replace-file-form"
          class="replace-file-dialog__confirm"
          :disabled="isReplacing"
          :aria-busy="isReplacing"
        >
          {{ isReplacing ? 'Subiendo…' : 'Subir nueva versión' }}
        </button>
      </div>
    </div>
  </dialog>
...
.replace-file-dialog::backdrop {
  background: rgba(15, 18, 25, 0.45);
}

.replace-file-dialog__box {
  padding: 1.5rem;
  background: var(--bg-surface);
  border-radius: var(--radius-lg);
}

.replace-file-dialog__title {
  margin: 0 0 0.5rem;
  font-size: 1.05rem;
  font-weight: 700;
...
.replace-file-dialog__notice {
  margin: 0 0 1rem;
  font-size: 0.85rem;
  color: var(--text-secondary);
}

.replace-file-dialog__config-error {
  display: flex;
...
.replace-file-dialog__retry-config {
  min-height: 44px;
...
.replace-file-dialog__retry-config:hover {
  background: var(--accent-hover);
}

.replace-file-dialog__form {
  display: flex;
  flex-direction: column;
  gap: 0.9rem;
}

.replace-file-dialog__limit {
  margin: 0;
  font-size: 0.85rem;
  color: var(--text-secondary);
}

.replace-file-dialog__dropzone {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 0.6rem;
...
.replace-file-dialog__dropzone--active {
  border-color: var(--accent);
  background: var(--accent-soft);
}

.replace-file-dialog__dropzone-hint {
  margin: 0;
  font-size: 0.85rem;
  color: var(--text-secondary);
}

.replace-file-dialog__choose-button {
  min-height: 44px;
  padding: 0.5rem 1.1rem;
  border-radius: var(--radius-sm);
  font-size: 0.9rem;
...
.replace-file-dialog__choose-button:hover:not(:disabled) {
  background: var(--bg-hover);
}

.replace-file-dialog__choose-button:disabled {
  opacity: 0.65;
  cursor: not-allowed;
}

.replace-file-dialog__selected {
  margin: 0;
  display: flex;
...
.replace-file-dialog__selected-name {
  min-width: 0;
  overflow: hidden;
...
.replace-file-dialog__selected-size {
  flex-shrink: 0;
  color: var(--text-secondary);
}
...
.replace-file-dialog__cancel,
.replace-file-dialog__confirm {
  min-height: 44px;
  padding: 0.5rem 1.1rem;
  border-radius: var(--radius-sm);
  font-size: 0.9rem;
...
.replace-file-dialog__cancel:hover:not(:disabled) {
  background: var(--bg-hover);
  color: var(--text-primary);
}
...
.replace-file-dialog__confirm:hover:not(:disabled) {
  background: var(--accent-hover);
}

.replace-file-dialog__cancel:disabled,
.replace-file-dialog__confirm:disabled {
  opacity: 0.65;
  cursor: not-allowed;
}
</style>
